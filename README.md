...
The custom implementation appears to be doing something right but the pretrained model is nearly perfect. A demo of both models is shown on [HuggingFace Spaces](https://huggingface.co/spaces/i4ata/CustomUnetSegmentation).

To train yourself with the parameters specified in `params.yaml` (Highly recommended to train on a machine with an available GPU):
